...
public class ExpExcelUtil {
	 ...
	 * @return
	 */
	public static HSSFWorkbook getWorkbook(String headStr, String sheetStr,
			List<String> titleList, List<List<String>> rowList) {
		WORK_BOOK = createExcel();
		/** 表头样式开始 */
		HSSFCellStyle headStyle = WORK_BOOK.createCellStyle();
		...
		HSSFFont headFont = WORK_BOOK.createFont();
		...
		/** 标题样式开始 */
		HSSFCellStyle titleStyle = WORK_BOOK.createCellStyle();
		...
		/** 内容样式开始 */
		HSSFCellStyle contentStyle = WORK_BOOK.createCellStyle();
		...
		/** 内容样式结束 */
		sheetStr = "".equals(sheetStr) ? DEFAULT_SHEET_NAME : sheetStr;
		HSSFSheet hssfSheet = WORK_BOOK.createSheet(sheetStr);
		int row = 0;// 创建的表格行号
		/** 创建表头合并单元格开始 */
		if (StringUtils.hasText(headStr)) {// 判断是否需要标题
			int length = titleList.size();
			length = length > 0 ? length - 1 : length;
			HSSFRow headRow = hssfSheet.createRow(row);// 表格头
			row++;
			HSSFCell headerCell = headRow.createCell(0);
			headerCell.setCellValue(headStr);
			headerCell.setCellStyle(headStyle);
			hssfSheet.autoSizeColumn(0);
			// 参数1：行号 参数2：起始列号 参数3：行号 参数4：终止列号
			//Region headRegion = new Region(0, (short) 0, 0, (short) length);
